...
当前 `GameplayView` 使用场景持有的棋盘底图和方块 `SpriteFrame`，棋盘底图采用 Cocos 九宫格 Sprite，四角和边框固定、中间区域随手机尺寸拉伸；通过常驻节点与 Sprite 池同步状态。棋盘空位由底图自身的格线显示，不铺设方块贴图。
